...
+(instancetype)keychainForForService:(NSString*)service
{
    static id sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[FXKeychain alloc] initWithService:service
                                                 accessGroup:nil];
    });
...
+(instancetype)keychainForGroup:(NSString*)group
{
    ...
    FXKeychain * chain=[dic objectForKey:[group md5]];
    if (chain==nil) {
        chain=[[FXKeychain alloc] initWithService:bundleID
                                      accessGroup:nil];
        [dic setObject:chain forKey:[group md5]];
    }
    return chain;
}
